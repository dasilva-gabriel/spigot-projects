...
public class SanctionGui_Rules extends Gui{
	
	public SanctionGui_Rules(Player player, Player target) {
	...
	public void drawScreen() {
		...
		setItem(13, new ItemBuilder(Material.STAINED_GLASS_PANE).setName(" ").setDamage(1).build());
		setItem(18, new ItemBuilder(Material.IRON_TRAPDOOR).setName("§8[§2»§8] §aRègles > SpawnKill").build());
		setItem(19, new ItemBuilder(Material.ENDER_PEARL).setName("§8[§2»§8] §aRègles > UseBug").build());
		setItem(20, new ItemBuilder(Material.EYE_OF_ENDER).setName("§8[§2»§8] §aRègles > Glitch").build());
		setItem(21, new ItemBuilder(Material.ANVIL).setName("§8[§2»§8] §aRègles > AntiJeu").build());
		setItem(22, new ItemBuilder(Material.BARRIER).setName("§8[§2»§8] §aRègles > Refus de Vérification").build());
	}
	...
	public void onClick(int position, ItemStack item, ClickType click) {
		...
		if(position == 18){
			...
			SanctionManager.sancPlayer(p, target, SancList.RULES_SPAWNKILL);
		}
		
		if(position == 19){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.RULES_USEBUG);
		}
		
		if(position == 20){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.RULES_GLITCH);
		}
		
		if(position == 21){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.RULES_ANTIJEU);
		}
		
		if(position == 22){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.RULES_RDF);
		}
	}
