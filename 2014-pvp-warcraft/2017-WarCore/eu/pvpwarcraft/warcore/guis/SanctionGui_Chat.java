...
public class SanctionGui_Chat extends Gui{
	
	public SanctionGui_Chat(Player player, Player target) {
	...
	public void drawScreen() {
		...
		setItem(11, new ItemBuilder(Material.STAINED_GLASS_PANE).setName(" ").setDamage(1).build());
		setItem(18, new ItemBuilder(Material.BOOK).setName("§8[§2»§8] §aChat > Spam").build());
		setItem(19, new ItemBuilder(Material.BOOK).setName("§8[§2»§8] §aChat > Flood").build());
		setItem(20, new ItemBuilder(Material.WRITTEN_BOOK).setName("§8[§2»§8] §aChat > Majuscules").build());
		setItem(21, new ItemBuilder(Material.BOOKSHELF).setName("§8[§2»§8] §aChat > Insultes").build());
		setItem(22, new ItemBuilder(Material.ENCHANTED_BOOK).setName("§8[§2»§8] §aChat > Langage").build());
	}
	...
	public void onClick(int position, ItemStack item, ClickType click) {
		...
		if(position == 18){
			...
			SanctionManager.sancPlayer(p, target, SancList.CHAT_SPAM);
		}
		if(position == 19){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.CHAT_FLOOD);
		}
		if(position == 20){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.CHAT_MAJUSCULES);
		}
		if(position == 21){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.CHAT_INSULTES);
		}
		if(position == 22){
			p.getOpenInventory().close();
			p.sendMessage("§8[§2✔§8] §aEnvoi de la sanction...");
			SanctionManager.sancPlayer(p, target, SancList.CHAT_LANGUAGE);
		}
	}
