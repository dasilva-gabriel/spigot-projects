...
public class MySQL {
	...
	private static String host = WarBungee.getInstance().getConfigurationManager().getSqlConfig().getString("host") + "/";
	private static String base = WarBungee.getInstance().getConfigurationManager().getSqlConfig().getString("database");
	private static String username = WarBungee.getInstance().getConfigurationManager().getSqlConfig().getString("username");
	private static String password = WarBungee.getInstance().getConfigurationManager().getSqlConfig().getString("password");
	...
	public static void tryConnect() {
		if (!isConnected())
			try {
				connection = DriverManager.getConnection(url + host + base + "?autoReconnect=true&useSSL=false", username, password);
			} catch (Exception e) {
				e.printStackTrace();
			}
	}
